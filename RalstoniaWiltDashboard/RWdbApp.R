        ...
        that filter will be removed."),
                   sidebarMenu(id = "sidebarid", 
                               ...
                               pickerInput(inputId = "vegprophost",
                                           ...
                                           choices = sort(unique(RSSC1$VPH2)),
                                           options = list(`live-search` = T,`actions-box` = T,size = 10,`selected-text-format` = "count > 1"),
                                           selected = sort(unique(RSSC1$VPH2)),
                                           multiple = T),
...
server <- function(input, output, session) {
  ...
  filter_multi <- function(data, column, values) {
    if (is.null(values) || length(values) == 0) return(data)
    col <- sym(column)
    data %>%
      filter(coalesce(as.character(!!col) %in% as.character(values), TRUE))
  }
  
  filter_map <- list(
    publication_year = "YearPub",
    isolation_year   = "YearIso",
    phylo            = "Phylotype3",
    sequevar         = "Sequevar4",
    pandemic_lineage = "PanLin",
    host_family      = "HostFam",
    host_species     = "HostSp",
    vegprophost      = "VPH2",
  ...
  filtered_data <- eventReactive(input$search, {
    ...
    cat("Start rows:", nrow(df), "\n")
    
    for (input_id in names(filter_map)) {
      colname <- filter_map[[input_id]]
      values  <- input[[input_id]]
      
      cat("\n--- Filter:", input_id, "on column", colname, "\n")
      cat("Selected values:", paste(values, collapse = ", "), "\n")
      cat("Unique data values (first 10):",
          paste(head(unique(df[[colname]]), 10), collapse = ", "), "\n")
      
      before <- nrow(df)
      df <- filter_multi(df, colname, values)
      after <- nrow(df)
      
      cat("Rows before:", before, "after:", after, "\n")
    }
